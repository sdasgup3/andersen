...
AliasResult AndersenAAResult::alias(const MemoryLocation& l1, const MemoryLocation& l2)
{
...
bool AndersenAAResult::pointsToConstantMemory(const MemoryLocation& loc, bool orLocal)
{
...
{
	result.reset(new AndersenAAResult(M, getAnalysis<TargetLibraryInfoWrapperPass>().getTLI()));
...
static RegisterPass<AndersenAAWrapperPass> X("anders-aa", "Andersen Alias Analysis", true, true);
